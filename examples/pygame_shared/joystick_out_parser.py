...
if (
    read_source_input
):  # this is a special variable that is set to true every time that the source is triggered and ready to read data

    read_source_input = (
        False  # it MUST be set to false at the end of the parser code
    )
    ...
    buttons = [
        usb_joystick.get_button(i) for i in range(joystick_buttons.shape[1])
    ]

    joystick_axis[:] = [ ax0, ax1 ]
