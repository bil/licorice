    ...
    while mIn.poll():
        ...
        if area == 176:  # knob turned
            if control == 3:  # knob 1
                ...
                print(
                    f"lowpass cutoff: {(low_pass_max - low_pass_min) * (value / 127.) + low_pass_min}",
                    flush=True,
                )
            ...
            if control == 9:  # knob 2
                ...
                print(
                    f"highpass cutoff: {(high_pass_max - high_pass_min) * (value / 127.) + high_pass_min}",
                    flush=True,
                )

            if control == 12:  # knob 3
                clip_ratio = 0.85 * (value / 127.0) + 0.1
            ...
            if control == 14:  # knob 5
                ...
                print(
                    f"pitch_shift value: {(pitch_shift_max - pitch_shift_min) * (value / 127.) + pitch_shift_min }",
                    flush=True,
                )
            ...
            if control == 15:  # knob 6
                ...
                print(
                    f"gain: {(gain_max - gain_min) * (value / 127.) + gain_min}db",
                    flush=True,
                )
...
if enable_distortion:
    ...
    np.clip(res, -1.0 * clip_val, clip_val, res)
    res += mean
