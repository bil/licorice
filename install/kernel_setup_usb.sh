...
# Kernel patch script for Ubuntu xenial-lts server (16.04) - USB support
...
cp $INSTALL_DIR/.config_usb $KERNEL_DIR/linux-4.4.12/.config
...
# build kernel
make-kpkg clean
CONCURRENCY_LEVEL=$NUM_CPUS fakeroot make-kpkg --initrd --append-to-version=-licorice-usb binary

# install kernel
sudo dpkg --force-confdef --force-confnew -i $KERNEL_DIR/linux-image-4.4.12-licorice-usb-rt19_4.4.12-licorice-usb-rt19-10.00.Custom_amd64.deb
sudo dpkg --force-confdef --force-confnew -i $KERNEL_DIR/linux-headers-4.4.12-licorice-usb-rt19_4.4.12-licorice-usb-rt19-10.00.Custom_amd64.deb
