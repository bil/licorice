...
def createTables(cur, table) :
  ...
  dir_path = os.getenv("BINARY_DIR") + "/" + dir_name
  if not (os.path.exists(dir_path)):
    return

  cur.execute("CREATE TABLE IF NOT EXISTS " + table + " (Filename TEXT, Timestamp DATETIME, File Contents BLOB)")
  ...
  for filename in os.listdir(dir_path) :
    ...
    cur.execute("INSERT INTO " + table + " VALUES (?,?,?)", (filename, mtime, packed_contents)) 
...
while (True): 
  ...
  if max(indices) == 0 : # first database still being created
    continue
  cur_index = indices.index(max(indices) - 1) # index in array corresponding to most recently closed db
