...
def __load_and_validate_model(file):
  ...
  if not working_abspath:
    ...
    working_abspath = os.path.join(os.path.abspath(working_abspath), "models")
  ...
  if not filepath:
    raise ValueError(f"Could not locate model file: {file}. Specify a full path or set LICORICE_MODEL_DIR")
...
def __get_licorice_paths():
  ...
  paths['modules'] = (
    os.environ.get("LICORICE_MODULE_DIR") or
    os.path.join(licorice_working_dir, 'modules')
  )
  paths['output'] = (
    os.environ.get("LICORICE_OUTPUT_DIR") or
    os.path.join(licorice_working_dir, 'run/out')
  )
  paths['export'] = (
    os.environ.get("LICORICE_EXPORT_DIR") or
    os.path.join(licorice_working_dir, 'run/export')
  )

  paths['tmp_modules'] = (
    os.environ.get("LICORICE_TMP_MODULE_DIR") or
    os.path.join(licorice_working_dir, '.modules')
  )
  paths['tmp_output'] = (
    os.environ.get("LICORICE_TMP_OUTPUT_DIR") or
    os.path.join(licorice_working_dir, 'run/.out')
  )
